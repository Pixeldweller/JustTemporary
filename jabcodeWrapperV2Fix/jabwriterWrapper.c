...
__declspec(dllexport) const char* __stdcall writeToTmpFile(struct jabDTO* ptrDTO) {
    ...
    {
		cleanMemory();
		reportErrorMod("Creating encode parameter failed -> Encoding kombination ungueltig");
        return "fehler";
    }
	...
	if(generateJABCode(enc, data) != 0)
	{
		reportErrorMod("Jabcode Generator Fehler -> Parameter kombination ungueltig oder Zeichen Anzahl zu groß für gewaehlte Dimensionen.");
		destroyEncode(enc);
		cleanMemory();
		//return 1;
		return "fehler";
...
__declspec(dllexport) const char* __stdcall readFromJabPNG(struct jabDTO* ptrDTO)
{
	...
	char* filepath = malloc(strlen(ptrDTO->filename)); /* make space for the new string (should check the return value ...) */
	strcpy(filepath, ptrDTO->filename);
...
__declspec(dllexport) const char* __stdcall getMasterSymbolEncodingCapacity(struct jabDTO* ptrDTO)
{
	...
	jab_int32 number = total_net_capacity;
	char* numberstring = malloc(100);
	sprintf(numberstring, "%d", total_net_capacity);
